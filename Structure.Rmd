...
This will be added later because there's only one strata. The following code works but it is not as informative.

```{r}
# poc_amova<-poppr.amova(pocillopora_cc, ~Region/Poblacion, 
poc_amova<-poppr.amova(pocillopora_cc, ~Poblacion, 
                       missing="ignore", 
                       within = T, clonecorrect = T)

poc_amova

# Permutation test
Aeutsignif<-randtest(poc_amova, nrepet = 999) # significancia
Aeutsignif
```
...
sfiles <- 
  list.files(path="C:/Users/Usuario/Desktop/Proyecto Structure/Tesis_final/tesis_final/50k100k/Results", 
...
# Load data K = 2
struc_k2<-read.csv("C:/Users/Usuario/Desktop/Proyecto Structure/structure_R_2.csv", sep = ";")
names(struc_k2)<-c("ID", "missing", "pop", 
                "cluster1", "cluster2")

# Load data K = 3
struc_k3<-read.csv("C:/Users/Usuario/Desktop/Proyecto Structure/structure_R_3.csv", sep = ";")
